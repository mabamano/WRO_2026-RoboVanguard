...
Raspberry Pi 5 Obstacle Challenge Autonomous Navigation (1-LAP PARKING TEST SCRIPT)

1-Lap Fast Parking Test Architecture:
- Runs 1 LAP (4 turns) of obstacle challenge navigation.
- Swapped Magenta Parking for RED PARKING BLOCKS:
  * Detects 2 Red Parking Blocks in ROI4.
  * Calculates the exact center X position between the 2 Red blocks.
  * Steers straight into the center between the 2 Red blocks for head-in parking.
  * Halts bot completely inside the parking space!
"""
...
def main():
    print("=" * 65)
    print("   ROBOVANGUARD - WRO Round 2 Obstacle Challenge (1-LAP PARKING TEST)")
    print("   Architecture: 1-Lap Run + Red Parking Block Center Guidance")
    print("=" * 65)
    ...
    window_name = "WRO Obstacle Challenge - Red Parking Test Monitor (Pi 5)"
    ...
    print("\n[READY] 1-Lap Red Parking Test Engine Ready!")
    ...
    print("[START] Driving FORWARD (1-Lap Test: Target = 4 Turns -> Red Block Parking)!")
    ...
    try:
        while True:
            ...
            contours_red = find_red_pillar_contours(img, ROI3)       # Red Pillars in upper ROI3
            contours_green = find_contours(img_lab, rGreen, ROI3)
            contours_orange = find_orange_line_contours(img, ROI4)    # Orange Lines in floor ROI4
            contours_blue = find_contours(img_lab, rBlue, ROI4)
            contours_magenta = find_contours(img_lab, rMagenta, ROI4)

            # Floor Red Contours for Red Parking Blocks (in floor ROI4)
            contours_red_parking = find_contours(img_lab, rRed, ROI4)
            ...
            #    (Guarded with t < TARGET_TURNS)
            # -------------------------------------------------------------
            if t < TARGET_TURNS and not isTurning and currTime >= lineLockoutUntil:
                if turnDir == "none":
            ...
            if isTurning:
            ...
            elif (t < TARGET_TURNS) and currTime >= turnCooldownUntil:
                ...
                if (lDetected or forced_dir != "none") and wallDropDetected:
                    targetTurnAngle = 60 if turnDir == "left" else 140
                    t += 1
                    print(f"[NAV EVENT] Marker Seen + Wall Drop! (L:{leftArea} R:{rightArea}) -> Triggering Turn ({t}/{TARGET_TURNS}) at speed {turnSpeed}...")
                    serial_ctrl.send_command(f"DRIVE:{turnSpeed}:{targetTurnAngle}")
                    last_cmd_time = currTime
                    last_drive_speed = turnSpeed
                    last_steer_angle = targetTurnAngle
            ...
            # -------------------------------------------------------------
            # 4. 1-LAP RED PARKING BLOCK ALGORITHM (t >= 4)
            # -------------------------------------------------------------
            if t >= TARGET_TURNS and not isTurning and not tempParking:
                print("=" * 65)
                print(f"[PARKING TEST] {TARGET_TURNS} turns (1 lap) complete! Searching for RED Parking Blocks...")
                print("=" * 65)
                tempParking = True

            if tempParking and not isTurning:
                # Find all valid Red Parking Blocks in floor ROI4
                valid_red_blocks = []
                for cnt in contours_red_parking:
                    area = cv2.contourArea(cnt)
                    if area > 100:  # Valid Red Parking Block area threshold
                        approx = cv2.approxPolyDP(cnt, 0.01 * cv2.arcLength(cnt, True), True)
                        x, y, w, h = cv2.boundingRect(approx)
                        cx = ROI4[0] + x + w // 2
                        cy_pos = ROI4[1] + y + h
                        valid_red_blocks.append((area, cx, cy_pos, w, h))

                if len(valid_red_blocks) >= 2:
                    # Sort blocks left to right by X coordinate
                    valid_red_blocks.sort(key=lambda b: b[1])
                    left_block_x = valid_red_blocks[0][1]
                    right_block_x = valid_red_blocks[-1][1]
                    
                    # Calculate EXACT CENTER X between the 2 Red Parking Blocks!
                    red_center_x = (left_block_x + right_block_x) // 2
                    
                    # Steer straight into the center between the 2 Red blocks (target center = 320)
                    error = 320 - red_center_x
                    steer_angle = int(100 - (error * 0.35))
                    steer_angle = max(60, min(140, steer_angle))

                    navMode = f"RED_PARKING_CENTER(Center_X:{red_center_x})"
                    print(f"[PARKING] Centering between 2 Red Blocks: L:{left_block_x} R:{right_block_x} -> Center_X:{red_center_x}, Steer:{steer_angle}")
                    serial_ctrl.send_command(f"DRIVE:{turnSpeed}:{steer_angle}")

                    # Park head-in when blocks reach bottom of frame or front US <= 20cm
                    max_y = max(b[2] for b in valid_red_blocks)
                    if max_y >= ROI4[3] - 15 or (f_us > 0 and f_us <= 20):
                        print("=" * 65)
                        print("[PARKING SUCCESS] PARKED STRAIGHT IN THE CENTER OF 2 RED PARKING BLOCKS!")
                        print("=" * 65)
                        serial_ctrl.send_command("STOP")
                        time.sleep(0.5)
                        break

                elif len(valid_red_blocks) == 1:
                    # 1 Red block visible -> Align offset from single block
                    b = valid_red_blocks[0]
                    cx = b[1]
                    target_x = 220 if cx < 320 else 420
                    error = target_x - cx
                    steer_angle = int(100 - (error * 0.25))
                    steer_angle = max(60, min(140, steer_angle))
                    navMode = f"RED_PARKING_SINGLE(X:{cx})"
                    print(f"[PARKING] Approaching Single Red Block: X={cx} -> Steer={steer_angle}")
                    serial_ctrl.send_command(f"DRIVE:{turnSpeed}:{steer_angle}")

                    if b[2] >= ROI4[3] - 15 or (f_us > 0 and f_us <= 20):
                        print("[PARKING SUCCESS] Parked alongside Red Parking Block!")
                        serial_ctrl.send_command("STOP")
                        time.sleep(0.5)
                        break
                else:
                    # Approach parking space straight forward
                    navMode = "PARKING_SEARCH"
                    serial_ctrl.send_command(f"DRIVE:{turnSpeed}:100")
            ...
            draw_roi(img_disp, ROI4, (0, 0, 255), 2)  # Red contour ROI for Parking
            ...
            draw_offset_contours(img_disp, contours_red_parking, ROI4, (0, 0, 255), 2)   # Red Parking Block contours
